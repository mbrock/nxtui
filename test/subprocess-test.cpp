...
static suite subprocess_tests{
    "PTY subprocesses", [] {
        "terminal session"_test = [] {
            "captures PTY output asynchronously"_test = [] {
                ...
                auto status = nxt::sync_wait(session.read_loop(
                    *scheduler, {}, [&damage_count] { ++damage_count; }));
                ...
                expect(
                    cell->fg.is_indexed() && cell->fg.c.indexed.idx == 1);
            };
            ...
            "writes input through the PTY"_test = [] {
                ...
                nxt::sync_wait(
                    session.write_all(*scheduler, "roundtrip\n"));
            ...
            "resizes the terminal and PTY together"_test = [] {
                auto session = nxt::subprocess::PtySession::spawn({
                ...
                expect(status.success());
                expect(
                    session.terminal().get_screen_text().find("7 23")
                    != std::string::npos);
            };
        };

        "child environment"_test = [] {
            "applies overlays"_test = [] {
                auto scheduler = nxt::scheduler::make_unique();
                auto session = nxt::subprocess::PtySession::spawn({
                    .argv =
                        {"/bin/sh",
                ...
                expect(status.success());
                expect(
                    session.terminal().get_screen_text().find(
                        "<xterm-256color><truecolor>")
                    != std::string::npos);
            };
        };

        "keyboard input"_test = [] {
            "encodes structured keys as shell PTY bytes"_test = [] {
                ...
                expect(
                    session.encode_key(ctrl_a) == std::string{"\x01", 1});
                ...
                expect(
                    session.encode_key(ctrl_k_with_base)
                    == std::string{"\x0b", 1});
