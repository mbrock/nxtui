...
static suite http_io_tests{
    "HTTP I/O", [] {
        ...
        "URLs"_test = [] {
            "recognize HTTP and HTTPS defaults"_test = [] {
                auto http_url =
                    io_http::parse_url("http://example.test/path"sv);
                expect(!http_url.tls);
                ...
                auto https_url =
                    io_http::parse_url("https://example.test:8443"sv);
        ...
        "response heads"_test = [] {
            "detect content length and chunked bodies"_test = [] {
                auto fixed = io_http::parse_response_head(
                    std::as_bytes(
                        std::span{
                            "HTTP/1.1 200 OK\r\nContent-Length: 11\r\n"sv}));
                expect(
                    io_http::response_status_text(fixed)
                    == "HTTP/1.1 200 OK");
                ...
                expect(
                    io_http::response_status_text(chunked)
                    == "HTTP/1.1 200 OK");
        ...
        "byte cursor"_test = [] {
            "parses complete in-memory slices"_test = [] {
                ...
                expect(
                    nxt::io::as_string_view(input.remaining()) == "hello");
            };
        };

        "byte reader"_test = [] {
            "refills until the slurp cut appears"_test = [] {
                ...
                auto reader =
                    nxt::io::byte_reader{transport, std::span{buffer}};
        ...
        "response bodies"_test = [] {
            "read content-length bodies from read-ahead and refills"_test =
                [] {
                    auto chunks = std::array{
                        "HTTP/1.1 200 OK\r\nContent-Length: 11\r\n\r\nhe"sv,
                        "llo world"sv,
                    ...
                    std::string body;
                    auto on_chunk = [&](std::span<const std::byte> chunk)
                        -> nxt::task<> {
                    ...
                    nxt::sync_wait(
                        io_http::read_content_length(reader, 11, on_chunk));

                    expect(body == "hello world");
                };

            "read chunked bodies from read-ahead and refills"_test = [] {
                auto chunks = std::array{
                ...
                nxt::sync_wait(io_http::read_chunked(reader, on_chunk));
        ...
        "requests"_test = [] {
            "return the head before streaming SSE bodies"_test = [] {
                auto sse =
                    "event: greeting\n"
                    "data: hello\n"
                    "\n"s;
                ...
                expect(
                    io_http::response_content_type_is(
                        head.head, "text/event-stream"));

                std::vector<nxt::http::server_sent_event> events;
                nxt::sync_wait([&]() -> nxt::task<> {
                    auto body =
                        io_http::http_body_reader{reader, head.head};
                    std::array<std::byte, 4096> body_buffer{};
                    auto body_reader =
                        nxt::io::byte_reader{body, std::span{body_buffer}};
                    while (auto event = co_await io_http::parse_sse_event(
                               body_reader))
            ...
            "collect non-streaming error bodies"_test = [] {
                auto response =
                ...
                auto body = nxt::sync_wait(
                    io_http::read_response_text(reader, head));
