...
void first_runtime_block_at(
    vterm::Terminal & term, row_t row, std::string_view text)
{
    ansi::mode = ansi::Mode::enabled;
    std::string buf;
    nxt::ansi::Writer w(buf);
    ...
    std::string_view text)
{
    ansi::mode = ansi::Mode::enabled;
    auto buf =
        rtty::append_block<rtty::ansi_string_backend>(partition, text);
    term.write(apply_onlcr(buf));
}
...
static suite regional_tty_tests{
    "Regional TTY", [] {
        "name the scroll margin for bottom-fixed partitions"_test = [] {
            auto partition =
            ...
            expect(
                partition.bottom_fixed.bottom_exclusive.index()
                == std::size_t{6});
            expect(partition.scroll.has_value());
            expect(
                partition.scroll->top_margin().index() == std::size_t{0});
            expect(
                partition.scroll->bottom_margin().index()
                == std::size_t{3});
        };
        ...
        "reserve incoming fixed regions when showing windows"_test = [] {
            auto hidden = rtty::screen_partition::for_bottom_fixed_height(
                6 * ln, 0 * ln);
            auto windowed = rtty::screen_partition::for_bottom_fixed_height(
                6 * ln, 2 * ln);
        ...
        "reserve from the old bottom margin while growing"_test = [] {
            auto short_hud =
                rtty::screen_partition::for_bottom_fixed_height(
                    6 * ln, 1 * ln);
        ...
        "release rows into scrollback while shrinking"_test = [] {
            ...
            expect(
                program[2].kind == rtty::command_kind::set_scroll_region);
            expect(
        ...
        "print complete appended block lines and leave the cursor on the next row"_test =
            [] {
                ...
                auto program =
                    rtty::append_block<rtty::command_list_backend>(
                        partition, "FIRST\nSECOND");
...
static suite compositor_tests{
    "Terminal compositor", [] {
        ...
        "renders column layout"_test = [] {
            ...
            term.write(
                render_to_string(compositor, layout, {20 * ch, 5 * ln}));
        ...
        "renders vterm screen layout"_test = [] {
            ...
            auto layout =
                tui::vterm_screen(source, tui::bg(Rgba8(10, 20, 30)));
            term.write(
                render_to_string(compositor, layout, {12 * ch, 3 * ln}));
        ...
        "renders vterm cursor overlay"_test = [] {
            ...
            auto layout = tui::vterm_screen(
                source, tui::bg(pane_bg) | tui::fg(Rgba8::white()));
        ...
        "renders styled text with colors"_test = [] {
            ...
            term.write(render_to_string(
                compositor,
                tui::text("Red", tui::fg(red)),
                {20 * ch, 1 * ln}));

            auto cell = term.get_cell(0, 0);
            expect(cell.has_value());
            expect(cell->fg.is_rgb() && cell->fg.c.rgb.red == 255)
                << "is red";
        };
    ...
    "HUD", [] {
        "placement"_test = [] {
            "appears at the bottom of the terminal"_test = [] {
                ...
                auto layout = tui::column(
                    tui::text("HUD-LINE-1"), tui::text("HUD-LINE-2"));
                term.write(render_to_string(
                    compositor, layout, {20 * ch, 2 * ln}));
      ...
      "HUD-LINE-2",  // row 5 ─┘
        ...
        "initial install"_test = [] {
            "scrolls existing bottom prompts above the HUD"_test = [] {
                ...
                write_at(
                    term, terminal_origin_v + 10 * ln, "PROMPT command");
            ...
            "leaves an empty first scrollback output row"_test = [] {
                ...
                append_runtime_block(
                    term, compositor.partition(), "FIRST BLOCK");
            ...
            "preserves insertion cursors above the fixed region"_test = [] {
                ...
                write_at(
                    term, terminal_origin_v + 2 * ln, "earlier output");
            ...
            "preserves wrapped shell commands through the first block"_test =
                [] {
                    ...
                    auto layout =
                        tui::column(tui::text("1hud"), tui::text("2hud"));
                    term.write(render_to_string(
                        compositor, layout, {20 * ch, 2 * ln}));
                    ...
                    append_runtime_block(
                        term, compositor.partition(), "1txt\n2txt");
        ...
        "rendering"_test = [] {
            ...
            "supports full-screen HUDs"_test = [] {
                ...
                auto layout = tui::column(
                    tui::text("A"), tui::text("B"), tui::text("C"));
                term.write(render_to_string(
                    compositor, layout, {20 * ch, 3 * ln}));
        ...
        "resizing"_test = [] {
            "clears rows freed by shrinking"_test = [] {
                ...
                auto tall_hud = tui::column(
                    ...
                    tui::text("OLD-4"));
                term.write(render_to_string(
                    compositor, tall_hud, {20 * ch, 4 * ln}));
                ...
                auto short_hud =
                    tui::column(tui::text("NEW-1"), tui::text("NEW-2"));
                term.write(render_to_string(
                    compositor, short_hud, {20 * ch, 2 * ln}));
            ...
            "preserves box footers at the old scroll bottom while growing"_test =
                [] {
                    ...
                    auto layout =
                        tui::column(tui::text("HUD-1"), tui::text("HUD-2"));
                    term.write(render_to_string(
                        compositor, layout, {20 * ch, 2 * ln}));
            ...
            "releases rows below existing log content while shrinking"_test =
                [] {
                    ...
                    append_runtime_block(
                        term, compositor.partition(), "NEXT");
...
static suite scroll_region_tests{
    "Scroll regions", [] {
        "scroll printed content without affecting the HUD"_test = [] {
            ...
            auto hud = tui::column(
                tui::text("HUD-LINE-1"), tui::text("HUD-LINE-2"));
            term.write(
                render_to_string(compositor, hud, {20 * ch, 2 * ln}));
    ...
    check_display (term, {
      ...
      "HUD-LINE-2",  // row 5
            ...
            // Third log line - oldest visible line scrolls off, bottom
            // stays blank
