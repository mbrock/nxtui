...
template<typename Layout>
Emphasis
rendered_emphasis(const Layout & layout, std::size_t row, std::size_t col)
{
    auto width = std::max<std::size_t>(
        1, static_cast<std::size_t>(layout.width_hint().min.count()));
    auto height = std::max<std::size_t>(
...
static suite llm_tests{
    "LLM UI", [] {
        ...
        "OpenAI Responses requests"_test = [] {
            "serialize response bodies"_test = [] {
                ...
                auto wire = nxt::http::serialize(
                    nxt::ai::responses::openai_responses_http_request(
                        request));
                ...
                expect(
                    json_at<std::string, "/model">(body) == "gpt-5-mini");
                expect(json_at<std::string, "/input">(body) == "Say ok.");
                expect(json_at<bool, "/stream">(body));
                expect(!json_at<bool, "/store">(body));
                expect(
                    json_at<std::string, "/reasoning/effort">(body)
                    == "minimal");
                expect(
                    json_at<std::string, "/reasoning/summary">(body)
                    == "auto");
                expect(
                    wire.find("Authorization: Bearer test-key\r\n")
                    != std::string::npos);
            };

            "serialize tools and function outputs"_test = [] {
                auto request = nxt::ai::responses::openai_responses_request{
                    .api_key = "test-key",
                    .model = "gpt-5-mini",
                    .input = "ignored for tool output turn",
                    .input_items = {nxt::ai::tools::function_call_output(
                        "call_123", "{\"ok\":true}")},
                ...
                auto body =
                    nxt::ai::responses::openai_responses_body(request);
                expect(
                    json_at<std::string, "/input/0/type">(body)
                    == "function_call_output");
                expect(
                    json_at<std::string, "/input/0/call_id">(body)
                    == "call_123");
                expect(
                    json_at<std::string, "/tools/0/name">(body)
                    == "nxt_echo");
                expect(
                    json_at<std::string, "/include/0">(body)
                    == "reasoning.encrypted_content");
                expect(
                    json_at<std::string, "/previous_response_id">(body)
                    == "resp_123");
                expect(json_at<bool, "/store">(body));
            };

            "derive function tool schemas"_test = [] {
                auto definition = glz::ex::write_json(
                    nxt::ai::tools::function_tool_definition(
                        test_echo_tool{}));

                expect(
                    json_at<std::string, "/type">(definition)
                    == "function");
                expect(
                    json_at<std::string, "/name">(definition)
                    == "nxt_echo");
                expect(
                    json_at<std::string, "/parameters/type">(definition)
                    == "object");
                expect(!json_has<"/parameters/title">(definition));
                expect(!json_at<bool, "/parameters/additionalProperties">(
                    definition));
                expect(
                    json_at<std::string, "/parameters/required/0">(
                        definition)
                    == "text");
                expect(
                    json_at<
                        std::string,
                        "/parameters/properties/text/type">(definition)
                    == "string");
                expect(
                    json_at<
                        std::string,
                ...
                auto empty_definition = glz::ex::write_json(
                    nxt::ai::tools::function_tool_definition(
                        test_empty_tool{}));
                auto properties =
                    glz::get_sv_json<"/parameters/properties">(
                        empty_definition);
                expect(bool(properties));
                expect(*properties == "{}");
                auto required = glz::get_sv_json<"/parameters/required">(
                    empty_definition);
                expect(bool(required));
                expect(*required == "[]");
            };

            "build stateless tool input history"_test = [] {
                auto request = nxt::ai::responses::openai_responses_request{
                    .api_key = "test-key",
                    .model = "gpt-5-mini",
                    .input = "Use a tool.",
                ...
                expect(
                    json_at<std::string, "/input/0/role">(body) == "user");
                expect(
                    json_at<std::string, "/input/1/type">(body)
                    == "function_call");
                expect(
                    json_at<std::string, "/input/2/type">(body)
                    == "function_call_output");
            };
        };

        "function calls"_test = [] {
            "parse and run matching tools"_test = [] {
                ...
                auto output = nxt::sync_wait(
                    nxt::ai::tools::run_function_tool(tools, *call));
                expect(!output.failed);
                expect(output.output == "hello");

                auto missing = *call;
                missing.name = "missing";
                auto error = nxt::sync_wait(
                    nxt::ai::tools::run_function_tool(tools, missing));
                expect(error.failed);
        ...
        "tool UI"_test = [] {
            "summarizes concrete tool parameters"_test = [] {
                ...
                expect(
                    nxt::ai::tool_ui::args_summary(tool_list, rg)
                    == "/needle/ in src");
                auto rg_display =
                    nxt::ai::tool_ui::display_for_call(tool_list, rg);
                ...
                expect(
                    nxt::ai::tool_ui::result_summary(rg_result)
                    == "8 bytes");
                ...
                expect(
                    nxt::ai::tool_ui::args_summary(tool_list, echo)
                    == "hello");
                auto echo_display =
                ...
                expect(
                    nxt::ai::tool_ui::args_summary(tool_list, bash)
                ...
                expect(
                    nxt::ai::tool_ui::result_summary(failed)
                    == "error: bad thing failed");
            };

            "drains fast subprocess output at EOF"_test = [] {
        ...
        "response items"_test = [] {
            "derive messages and calls"_test = [] {
            ...
            "read output item types"_test = [] {
                ...
                expect(
                    nxt::ai::response_turn::output_item_type(event)
                    == "reasoning");
            };
        };

        "response text"_test = [] {
            "preserves paragraph breaks while wrapping"_test = [] {
                ...
                expect(
                    lines
                    == std::vector<std::string>{
                        "alpha beta", "", "second paragraph"});
            };

            "wraps by terminal cell width"_test = [] {
                auto lines = nxt::ai::response_turn::wrap_stream_text(
                    "aa \xe7\x95\x8c bb", 5);
            ...
            "indents markdown list continuations"_test = [] {
                auto lines = nxt::ai::response_turn::wrap_stream_text(
                    "- alpha beta gamma", 10);
            ...
            "parses inline bold and code spans"_test = [] {
            ...
            "strips bold paragraph markers without padding"_test = [] {
        ...
        "folded blocks"_test = [] {
            "fold finished thoughts to headings"_test = [] {
                auto layout =
                    nxt::ai::response_turn::folded_thought_block();
            ...
            "fold finished tool results to done cards"_test = [] {
        ...
        "HUD block state"_test = [] {
            "renders folded rows above active work"_test = [] {
                ...
                expect(
                    render_lines(layout)
                    == std::vector<std::string>{
                        "▸ thought     folded", "working"});
            };

            "tail-scrolls when constrained"_test = [] {
                auto hud = nxt::ai::hud_blocks::State{};
            ...
            "clips oversized newest rows instead of hiding them"_test = [] {
                auto hud = nxt::ai::hud_blocks::State{};
                hud.add(nxt::tui::text("old"));

                auto layout = hud.view(
                    nxt::tui::column(
        ...
        "output separator"_test = [] {
            "is one horizontal rule line"_test = [] {
                auto layout = nxt::ai::response_turn::output_separator();

                expect(layout.height_hint().min == 1 * ln);
                expect(
                    render_lines(layout) == std::vector<std::string>{"─"});
            };
        };

        "OpenAI Responses streams"_test = [] {
            "emit raw SSE JSON events"_test = [] {
                auto sse =
                ...
                nxt::sync_wait([&]() -> nxt::task<> {
                    auto stream =
                        nxt::ai::responses::openai_response_stream<
                ...
                expect(transport.written().starts_with(
                    "POST /v1/responses HTTP/1.1\r\n"));
            };

            "leave event data unparsed"_test = [] {
                auto sse =
                    "event: response.output_text.delta\n"
                    "data: not-json\n"
                    "\n"
                ...
                auto event = nxt::sync_wait(
                    [&]() -> nxt::task<std::optional<
                              nxt::ai::responses::stream_event>> {
                        auto stream =
                            nxt::ai::responses::openai_response_stream<
                                nxt::io::string_transport>{transport};
                        co_await stream.connect(request);
