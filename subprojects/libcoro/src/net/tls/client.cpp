...
client::client(
    std::unique_ptr<coro::scheduler>& scheduler, std::shared_ptr<context> tls_ctx, const net::socket_address& endpoint)
    : client(scheduler, std::move(tls_ctx), endpoint, {})
{
}
...
client::client(
    coro::scheduler* scheduler, std::shared_ptr<context> tls_ctx, net::socket socket, const net::socket_address& endpoint)
...
client::client(client&& other) noexcept
    : m_scheduler(std::exchange(other.m_scheduler, nullptr)),
      ...
      m_server_name(std::move(other.m_server_name)),
...
auto client::handshake(std::chrono::milliseconds timeout) -> coro::task<connection_status>
{
    ...
    if (!m_server_name.empty() && SSL_set_tlsext_host_name(tls, m_server_name.c_str()) != 1)
    {
        co_return connection_status::handshake_failed;
    }

    if (!m_server_name.empty() && SSL_set1_host(tls, m_server_name.c_str()) != 1)
    {
        co_return connection_status::handshake_failed;
    }
