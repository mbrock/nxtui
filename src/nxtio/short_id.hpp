...
/// cryptographic randomness (`getrandom`). Crockford excludes the
/// visually-confusable letters `I`, `L`, `O`, `U`, so the produced ids
/// can be read aloud or typed without ambiguity. 40 bits of entropy is
/// not safe against an adversary, but is more than enough for spans
/// and run ids within a single process run.
