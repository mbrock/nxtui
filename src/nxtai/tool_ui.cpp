...
namespace nxtai {
namespace {
nxtrt::task<nxtai::tools::tool_result> run_one_tool_call_worker(
...
nxtrt::task<nxtrt::catching_deed<tool_result>>
    ...
    bool & done)
{
    ...
    while (!done) {
        ...
        co_await nxtrt::draw(nxtai::tool_tui::render_call(view));
        co_await nxtrt::op::timeout::after(frame_interval);
    }
    co_return std::move(deed);
}

nxtrt::task<nxtai::tools::function_call_result> run_one_tool_call_ui(
    const tool_registry & tools,
    ...
    std::chrono::milliseconds settle_delay)
{
    ...
    auto done = false;
    auto worker = co_await nxtrt::with_zone(
        [&] {
    ...
    if (settle_delay > std::chrono::milliseconds{0})
        co_await nxtrt::op::timeout::after(settle_delay);
...
nxtrt::task<nxtai::tools::function_call_result>
...
nxtrt::task<std::vector<nxtrt::catching_deed<function_call_result>>>
spawn_tool_call_children(
    const tool_registry & tools,
    std::vector<function_call> & calls,
    std::vector<nxtrt::widget_slot> & surfaces,
    std::chrono::milliseconds settle_delay,
    std::size_t & done_count)
{
    auto out = std::vector<nxtrt::catching_deed<function_call_result>>{};
    ...
    co_await nxtrt::draw(nxtrt::child_slots_column(surfaces));
    ...
    std::chrono::milliseconds settle_delay)
{
    ...
    auto surfaces = std::vector<nxtrt::widget_slot>{};
